...
def normalize_message(msg: str) -> str:
    ...
    if contains_hindi:
        try:
            msg = transliterate(msg, sanscript.DEVANAGARI, sanscript.ITRANS).lower()
        except:
            pass
    return msg
...
def get_reply(user_msg: str) -> str:
    ...
    # ✅ Case 1: Only emojis
    only_emojis = all(char in emoji.EMOJI_DATA for char in msg) and not any(c.isalnum() for c in msg)
    if only_emojis:
        for char in msg:
    ...
    # ✅ Case 2: Text + emoji
    first_emoji = next((char for char in msg if char in emoji.EMOJI_DATA), None)
    text_only = ''.join(char for char in msg if char not in emoji.EMOJI_DATA).strip()
    ...
    if text_normalized in conversations:
        return conversations[text_normalized] + (f" {first_emoji}" if first_emoji else "")
    ...
    if highest_score >= SIMILARITY_THRESHOLD:
        return conversations[best_match] + (f" {first_emoji}" if first_emoji else "")
